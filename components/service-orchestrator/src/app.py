...
async def deploy_service(service_request: ServiceRequest = Body(...)):
    ...
    if response_content:
        ...
        file_like_object = BytesIO(base64.b64decode(
            response_content).decode().encode('utf-8'))
        ...
        return JSONResponse(content={"message": "Received", "status": "deployed", "service_name": service_name, "file_name": file_name, "site_id": site_id, "iml_endpoint": iml_endpoint})
    else:
...
async def get_requests():
    return JSONResponse(content={"deployed_services": list(request_states.keys())})
...
async def get_request_by_id(request_id: str):
    if request_id not in request_states:
        raise HTTPException(status_code=404, detail="Service not found")
    return JSONResponse(content={"service_name": request_id, "details": request_states[request_id]})
